...
public class FirstPersonController : MonoBehaviour
{
    ...
    private GameObject lastHighlightedTrash;
    ...
    private void Update()
    {
        ...
        {
            //Ray rayy = playerCamera.ScreenPointToRay(Input.mousePosition);
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 100f))
        {
            ...
            if (hit.collider.CompareTag("Trash"))
            {
                ...
                if (lastHighlightedTrash != trash)
                {
                    // Remove highlight from previous
                    if (lastHighlightedTrash != null && originalTrashMaterial != null)
                        lastHighlightedTrash.GetComponent<Renderer>().material = originalTrashMaterial;

                    // Store original and highlight new
                    originalTrashMaterial = trash.GetComponent<Renderer>().material;
                    trash.GetComponent<Renderer>().material = highlightMaterial;
                    lastHighlightedTrash = trash;
                }
            }
            else
            {
                // Remove highlight if not pointing at trash
                if (lastHighlightedTrash != null && originalTrashMaterial != null)
                {
                    lastHighlightedTrash.GetComponent<Renderer>().material = originalTrashMaterial;
                    lastHighlightedTrash = null;
                    originalTrashMaterial = null;
                }
            }
        }
        else
        {
            // Remove highlight if not hitting anything
            if (lastHighlightedTrash != null && originalTrashMaterial != null)
            {
                lastHighlightedTrash.GetComponent<Renderer>().material = originalTrashMaterial;
                lastHighlightedTrash = null;
